...
function formatTimestamp(value) {
  if (!value) return '历史计划'
  const date = new Date(value)
  if (Number.isNaN(date.getTime())) return '历史计划'
...
Page({
  ...
  async loadData(force = false) {
    ...
    try {
      ...
      else this.setData({ viewState: 'error', offline: true, errorMessage: error.message || '历史计划加载失败，请重试' })
    } finally {
  ...
  async refreshAfterConflict() {
    ...
    wx.showModal({
      title: '计划历史已变化',
      content: refreshed ? '已重新载入云端历史，请核对后再恢复。当前计划没有被替换。' : '暂时无法刷新云端历史，请恢复网络后重试。当前计划没有被替换。',
      showCancel: false,
      confirmText: '知道了',
    })
  },
  ...
  async restorePlan(event) {
    ...
    const confirmed = await confirmModal({
      title: '恢复这份历史计划？',
      content: `将“${plan.title}”设为当前计划。现在使用的计划会自动进入历史，并恢复这份计划自己的采购勾选和晚餐选择。`,
      confirmText: '继续',
    })
    if (!confirmed) return
    const confirmedAgain = await confirmModal({
      title: '再次确认恢复',
      content: '确定切换当前计划吗？此操作不会删除其他历史计划。',
      confirmText: '确认恢复',
    })
    ...
    try {
      ...
      wx.showToast({ title: '历史计划已恢复', icon: 'success' })
      setTimeout(() => wx.switchTab({ url: '/pages/plan/plan' }), 350)
    } catch (error) {
      if (isConflict(error)) await this.refreshAfterConflict()
      else wx.showModal({ title: '恢复失败', content: `${error.message || '请稍后重试'}。当前计划没有变化。`, showCancel: false, confirmText: '知道了' })
    } finally {
