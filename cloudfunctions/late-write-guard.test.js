...
assert(functionBody(health, 'finalizePhoto').includes('if (uploadedFileId) await deletePrivateFiles([uploadedFileId])'), '健康照片永久文件上传后的失败路径必须补偿删除')
assert(functionBody(health, 'finalizePhoto').includes('cleanupPhotoTicket(openid, prepared.token)'), '照片失败路径必须登记并领取票据清理')
